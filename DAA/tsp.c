//Implementation of Travelling Salesman Problem (TSP) 

/*
Aim : To write a program to solve the Travelling Salesman Problem (TSP) and find the minimum cost path that visits all cities   
...
// S will have 2^n possible combinations of cities, so the size of it should be 2^MAXN that is (1 << MAXN)
int dp[MAXN][1 << MAXN];
...
int g(int i, int S)
{
    ...
    // You can also start the loop from k = 1 to k < n, since city 0 (A) is not included in S.
